...
export const ERROR_PATTERNS: ErrorPattern[] = [
  ...
  },
  {
    ...
    category: "scss-variable",
    pattern: /key\s+['"]?(\S+?)['"]?\s+not.*found.*map|map.*missing.*key\s+['"]?(\S+?)['"]?|undefined.*map.*key\s+['"]?(\S+?)['"]?/i,
    description: "The key does not exist in the SCSS map",
    fixTemplate: "Add key '$1' to the map or use map-keys($map) to check available keys",
  ...
  // =========================================================================
  {
    ...
    category: "scss-mixin",
    pattern: /undefined.*mixin[:\s]+(\S+)|mixin\s+['"]?(\S+?)['"]?\s+(?:not.*defined|is.*undefined)|no.*mixin.*named\s+['"]?(\S+)/i,
    description: "The mixin has not been defined",
    fixTemplate: "Define the mixin: @mixin $1 { } or import the file containing it",
  ...
  },
  {
    ...
    pattern: /undefined.*function[:\s]+(\S+)|function\s+['"]?(\S+?)['"]?\s+(?:not.*defined|is.*undefined)|no.*function.*named\s+['"]?(\S+)/i,
    description: "The SCSS function has not been defined",
    fixTemplate: "Define the function: @function $1() { @return value; }",
