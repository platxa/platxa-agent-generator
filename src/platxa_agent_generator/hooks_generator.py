...
def generate_session_start_context_script(
    ...
    """Generate a SessionStart hook script that loads instincts and progress as additionalContext.
...
INSTINCTS_DIR_ENV="$INSTINCTS_DIR" PROGRESS_FILE_ENV="$PROGRESS_FILE" \\
    MAX_CHARS_ENV="$MAX_CHARS" AGENT_NAME="{agent_name}" \\
    python3 - <<'PYEOF'
import os
import sys
from pathlib import Path
...
if not instincts_text and not progress_text:
    sys.exit(0)

sections = []
instincts_budget = max_chars

if instincts_text:
    if len(instincts_text) > instincts_budget:
        instincts_text = (
            instincts_text[:instincts_budget - 80]
