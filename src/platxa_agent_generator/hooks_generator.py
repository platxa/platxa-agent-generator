...
def generate_stop_observation_script(
    agent_name: str,
    observations_file: str = ".claude/observations.jsonl",
) -> str:
    """Generate a bash script that emits a structured JSONL observation on Stop.

    The Stop hook fires when Claude Code is about to end a session. This script
    writes one ``ObservationRecord``-compatible JSON row to ``observations_file``
    and prints an ``additionalContext`` advisory directing the model to dispatch
    the observer-subagent for deeper transcript analysis.

    The emitted JSONL row contains the required ``ObservationRecord`` fields
    (timestamp, tool, input_summary, project_id, project_name) plus optional
    fields (session_id, agent_name, type, evidence) so the downstream
    continuous-learning pipeline can ingest the row without transformation.

    Args:
        agent_name: Agent name included in the observation row and log messages.
        observations_file: Path to the JSONL observation log (default
            ``.claude/observations.jsonl``). The parent directory is created
            at runtime if missing.

    Raises:
        ValueError: if ``agent_name`` fails the standard name validator or
            ``observations_file`` is empty / whitespace-only.

    Returns:
        Complete bash script as a string.
    """
    _validate_agent_name(agent_name)
    if not observations_file or not observations_file.strip():
        raise ValueError("observations_file must be a non-empty string")
    quoted_obs_file = shlex.quote(observations_file)
    script = f"""#!/usr/bin/env bash
# Stop observation hook for {agent_name}
# Generated by platxa-agent-generator hooks_generator.py
#
# Emits a structured JSONL observation row on session end and prints an
# additionalContext advisory to dispatch the observer-subagent.
...
#       "hooks": [{{
#         "type": "command",
#         "command": "/path/to/{agent_name}-observe-stop.sh"
#       }}]
#     }}]
#   }}
...
if command -v python3 &>/dev/null; then
    OBS_FILE_ENV="$OBS_FILE" AGENT_NAME="{agent_name}" \\
        python3 - <<'PYEOF'
import json
import os
import sys
...
obs_file = os.environ.get("OBS_FILE_ENV", ".claude/observations.jsonl")
...
record = {{
    "timestamp": timestamp,
    "tool": "Stop",
    "input_summary": f"Session ended for {{agent_name}}",
    "project_id": project_dir,
    "project_name": project_name,
    "session_id": session_id,
    "agent_name": agent_name,
    "type": "session_end",
    "evidence": "Stop hook fired at session boundary",
    "examples": [],
    "outcome": "completed",
    "confidence": 1.0,
    "promoted_to": None,
...
try:
    ...
    sys.stderr.write(f"[{{agent_name}}] could not write observation: {{e}}\\n")
PYEOF
else
    echo "[{agent_name}] python3 not found; skipping observation JSONL emission" >&2
fi

echo "[{agent_name}] Stop observation: dispatch observer-subagent for transcript analysis."
...
def generate_stop_observation_hook_config(agent_name: str, script_path: str) -> dict[str, Any]:
    ...
    Args:
        agent_name: Agent name — validated for non-empty at the API boundary.
        script_path: Absolute path to the observation script.
...
def generate_observer_dispatch_script(
    agent_name: str,
    lock_dir: str = ".claude/hooks",
) -> str:
    """Generate a SubagentStop hook script with 5-layer re-entrancy guard.

    The script checks all 5 guard layers before dispatching the observer
    subagent. If any layer blocks, the script emits a reason on stderr
    and exits 0 (non-blocking). On success it sets PLATXA_OBSERVER_ACTIVE=1,
    records the dispatch timestamp, and prints an additionalContext advisory.

    Args:
        agent_name: Agent name for log messages and script header.
        lock_dir: Directory for lockfile and timestamp file (default
            ``.claude/hooks``).
    ...
    """
    _validate_agent_name(agent_name)
    quoted_lock_dir = shlex.quote(lock_dir)
    script = f"""#!/usr/bin/env bash
# SubagentStop observer dispatch with 5-layer re-entrancy guard for {agent_name}
# Generated by platxa-agent-generator hooks_generator.py
#
# Checks 5 independent guard layers before dispatching the observer-subagent.
# If any layer blocks, emits reason on stderr and exits 0 (non-blocking).
#
# Guard layers:
#   1. Env var PLATXA_OBSERVER_ACTIVE
#   2. Lockfile (.observer.lock)
#   3. stop_hook_active flag in payload
#   4. Transcript source (observer cannot trigger itself)
#   5. Min elapsed time (30s cooldown)
#
# Usage in settings.json:
#   "hooks": {{
#     "SubagentStop": [{{
#       "hooks": [{{
#         "type": "command",
#         "command": "/path/to/{agent_name}-observer-dispatch.sh"
#       }}]
#     }}]
#   }}
...
# Read stdin payload once (Claude Code passes JSON via stdin)
TOOL_INPUT=$(cat 2>/dev/null || echo "{{}}")
...
if command -v python3 &>/dev/null; then
    LOCK_DIR_ENV="$LOCK_DIR" TOOL_INPUT_ENV="$TOOL_INPUT" \\
        python3 - <<'PYEOF'
import json
import os
import sys
...
lock_dir = Path(os.environ.get("LOCK_DIR_ENV", ".claude/hooks"))
try:
    payload = json.loads(os.environ.get("TOOL_INPUT_ENV") or "{{}}")
...
# All layers passed — record dispatch (acquires lockfile for layer 2)
record_dispatch(lock_dir)
print(
    f"[{{agent_name or 'observer'}}] SubagentStop guard passed: "
    f"dispatch observer-subagent for transcript analysis."
)
PYEOF

# Layer 1 activation: export env var so child processes see it
export PLATXA_OBSERVER_ACTIVE=1
else
    echo "[{agent_name}] python3 not found; skipping observer dispatch" >&2
fi
...
def generate_observer_dispatch_hook_config(agent_name: str, script_path: str) -> dict[str, Any]:
    """Generate settings.json hook config for SubagentStop observer dispatch.

    Args:
        agent_name: Agent name — validated at the API boundary.
        script_path: Absolute path to the dispatch script.

    Raises:
        ValueError: if agent_name or script_path is empty / whitespace-only.

    Returns:
        Dict in settings.json hooks format for SubagentStop event.
    """
    _validate_agent_name(agent_name)
    if not script_path or not script_path.strip():
        raise ValueError("script_path must be a non-empty string")
...
def generate_session_start_context_script(
    agent_name: str,
    ...
    """Generate a SessionStart hook script that loads instincts, progress, and resume state.

    The script recursively reads all ``.md`` files under ``instincts_dir``,
    concatenates them with ``progress_file`` content, and emits text on
    stdout that Claude Code injects as ``additionalContext`` — capped at
    ``max_chars``. Instincts are prioritized: when the
    combined output exceeds the cap, progress entries are truncated first.
    If the full context still exceeds the cap after dropping all progress,
    instinct text is truncated with an overflow notice.

    When ``source=resume`` is detected in the SessionStart hook JSON
    (provided by Claude Code on stdin), the script also loads the prior
    workflow state from ``state_dir`` via
    :class:`~platxa_agent_generator.state_persistence.StatePersistence`
    and emits a ``[workflow-state]`` section with the last phase,
    next phase, and completed checkpoints so the model can resume
    where the previous session left off.

    The overflow notice includes the path to the instincts directory so the
    model can Read the full files on demand.

    Args:
        agent_name: Agent name for log messages and script comments.
        instincts_dir: Directory to scan recursively for ``.md`` instinct
            files (default ``.claude/instincts``).
        progress_file: Path to the progress log file (default
            ``.claude/claude-progress.txt``).
        max_chars: Character budget for the ``additionalContext`` payload
            (default 10000). Must be >= 100.
        state_dir: Project root directory where ``.claude/state/`` lives.
            Used to locate the persisted ``SessionState`` on resume
            (default ``"."``).

    Raises:
        ValueError: if ``agent_name`` fails the standard name validator,
            ``instincts_dir`` or ``progress_file`` is empty / whitespace,
            ``state_dir`` is empty / whitespace, or ``max_chars`` < 100.

    Returns:
        Complete bash script as a string.
    """
    _validate_agent_name(agent_name)
    if not instincts_dir or not instincts_dir.strip():
        raise ValueError("instincts_dir must be a non-empty string")
    if not progress_file or not progress_file.strip():
    ...
    if max_chars < 100:
        raise ValueError(f"max_chars must be >= 100 (got {max_chars})")
    quoted_instincts_dir = shlex.quote(instincts_dir)
    quoted_progress_file = shlex.quote(progress_file)
    quoted_state_dir = shlex.quote(state_dir)
    script = f"""#!/usr/bin/env bash
# SessionStart context injection hook for {agent_name}
# Generated by platxa-agent-generator hooks_generator.py
#
# Loads instincts from {instincts_dir}/**/*.md and recent progress from
# {progress_file}, then emits them as additionalContext on stdout.
# On source=resume, also loads prior workflow state from {state_dir}/.claude/state/.
# Instincts are prioritized; progress is truncated first when the combined
# output exceeds {max_chars} chars. Exit code is always 0 (non-blocking).
#
# Usage in settings.json:
#   "hooks": {{
#     "SessionStart": [{{
#       "hooks": [{{
#         "type": "command",
#         "command": "/path/to/{agent_name}-session-context.sh"
#       }}]
#     }}]
#   }}
...
INSTINCTS_DIR={quoted_instincts_dir}
PROGRESS_FILE={quoted_progress_file}
STATE_DIR={quoted_state_dir}
MAX_CHARS={max_chars}

# Capture stdin (Claude Code passes SessionStart JSON here)
STDIN_JSON=$(cat)

if ! command -v python3 &>/dev/null; then
    echo "[{agent_name}] session-context: python3 required but not found; skipping context injection." >&2
    exit 0
fi

INSTINCTS_DIR_ENV="$INSTINCTS_DIR" PROGRESS_FILE_ENV="$PROGRESS_FILE" \\
    STATE_DIR_ENV="$STATE_DIR" MAX_CHARS_ENV="$MAX_CHARS" \\
    AGENT_NAME="{agent_name}" STDIN_JSON_ENV="$STDIN_JSON" \\
    python3 - <<'PYEOF'
import json
import os
import sys
...
agent_name = os.environ.get("AGENT_NAME", "")
instincts_dir = os.environ.get("INSTINCTS_DIR_ENV", ".claude/instincts")
progress_file = os.environ.get("PROGRESS_FILE_ENV", ".claude/claude-progress.txt")
state_dir = os.environ.get("STATE_DIR_ENV", ".")
max_chars = int(os.environ.get("MAX_CHARS_ENV", "10000"))

stdin_raw = os.environ.get("STDIN_JSON_ENV", "{{}}")
try:
...
if is_resume:
    try:
        from platxa_agent_generator.state_persistence import StatePersistence, resume_phase
        sp = StatePersistence(base_dir=state_dir)
        loaded = sp.load()
        if loaded:
            parts = []
            parts.append(f"Resuming session — last phase: {{loaded.workflow_phase}}")
            next_phase = resume_phase(loaded)
            if next_phase:
                parts.append(f"Next phase to run: {{next_phase}}")
            if loaded.checkpoints:
                completed = [cp.phase for cp in loaded.checkpoints]
                parts.append(f"Completed phases: {{', '.join(completed)}}")
                last_cp = loaded.checkpoints[-1]
                if last_cp.phase_data:
                    parts.append(f"Last checkpoint data: {{json.dumps(last_cp.phase_data)}}")
            if loaded.workflow_data:
                wd = json.dumps(loaded.workflow_data, indent=2)
                parts.append(f"Workflow data:\\n{{wd}}")
            if loaded.metadata.agent_name:
                parts.append(f"Agent: {{loaded.metadata.agent_name}}")
            workflow_state_text = "\\n".join(parts)
    except ImportError:
        print(
            f"[{{agent_name}}] platxa_agent_generator not installed; "
            "skipping workflow state restoration",
            file=sys.stderr,
        )
    except Exception as exc:
        print(
            f"[{{agent_name}}] failed to load workflow state: {{exc}}",
            file=sys.stderr,
        )

instincts_text = ""
instinct_path = Path(instincts_dir)
if instinct_path.is_dir():
    files = sorted(instinct_path.rglob("*.md"))
    parts = []
    for fp in files:
        try:
            content = fp.read_text(encoding="utf-8").strip()
            if content:
                parts.append(f"--- {{fp.relative_to(instinct_path)}} ---\\n{{content}}")
        except OSError:
            pass
    instincts_text = "\\n\\n".join(parts)
...
if not instincts_text and not progress_text and not workflow_state_text:
    sys.exit(0)

sections = []

if workflow_state_text:
    wf_budget = min(2000, max_chars // 4)
    if len(workflow_state_text) > wf_budget:
        cut = max(0, wf_budget - 40)
        workflow_state_text = workflow_state_text[:cut] + "\\n[TRUNCATED]"
    sections.append(f"[workflow-state]\\n{{workflow_state_text}}")

instincts_budget = max(0, max_chars - sum(len(s) for s in sections))

if instincts_text:
    if len(instincts_text) > instincts_budget:
        instincts_text = (
            instincts_text[:max(0, instincts_budget - 80)]
            + f"\\n\\n[TRUNCATED — full instincts at {{instincts_dir}}/]"
        )
    sections.append(f"[instincts]\\n{{instincts_text}}")

remaining = max_chars - sum(len(s) for s in sections)

if progress_text and remaining > 100:
    if len(progress_text) > remaining:
        progress_text = (
            progress_text[-(remaining - 60):]
        )
...
def generate_session_start_context_hook_config(
    agent_name: str,
    script_path: str,
) -> dict[str, Any]:
    """Generate Claude Code settings.json hook config for SessionStart context injection.

    Args:
        agent_name: Agent name — validated for non-empty at the API boundary.
        script_path: Absolute path to the context injection script.

    Raises:
        ValueError: if agent_name or script_path is empty / whitespace-only.

    Returns:
        Dict in settings.json hooks format for SessionStart event.
    """
    _validate_agent_name(agent_name)
    if not script_path or not script_path.strip():
        raise ValueError("script_path must be a non-empty string")
...
def generate_stop_hook_completion_check(
    agent_name: str,
    max_iterations: int = 5,
) -> str:
    """Generate a Stop-hook script that gates termination on a completion-promise marker.

    The script reads the Claude Code Stop-hook JSON payload from stdin,
    regex-extracts the inner token from ``<promise>(.*?)</promise>`` in
    ``last_assistant_message``, and compares it to ``"COMPLETE"`` via
    byte-exact equality (the matching protocol defined in
    ``shared/constants.py``).

    Exit codes per Claude Code convention:
    - 0: allow termination (marker present OR iteration >= max)
    - 2: block + re-prompt (marker absent AND iteration < max)

    When blocking (exit 2), stderr carries a structured reason so the
    model sees actionable feedback on the next turn.

    Args:
        agent_name: Agent name for log messages and script comments.
        max_iterations: Safety-valve cap. When ``current_iteration`` in the
            payload meets or exceeds this value the script allows termination
            regardless of whether the marker is present. Default 5.
    ...
    inner_token = re.search(r"<promise>(.*?)</promise>", COMPLETION_PROMISE_MARKER)
    expected_token = inner_token.group(1) if inner_token else "COMPLETE"

    script = f"""#!/usr/bin/env bash
# Stop-hook completion check for {agent_name}
# Generated by platxa-agent-generator hooks_generator.py
#
# Gates agent termination on the presence of a <promise>COMPLETE</promise>
# marker in last_assistant_message. Uses regex extraction + byte-exact
# comparison (not naive substring grep) per the matching protocol in
# shared/constants.py.
#
# Exit 0 = allow termination, Exit 2 = block + re-prompt.
#
# Usage in settings.json:
#   "hooks": {{
#     "Stop": [{{
#       "hooks": [{{
#         "type": "command",
#         "command": "/path/to/{agent_name}-completion-check.sh"
#       }}]
#     }}]
#   }}
...
TOOL_INPUT_ENV="$TOOL_INPUT" MAX_ITER="$MAX_ITERATIONS" AGENT_NAME="{agent_name}" \\
...
agent = os.environ.get("AGENT_NAME", "")
expected = os.environ.get("EXPECTED_TOKEN", "COMPLETE")
max_iter = int(os.environ.get("MAX_ITER", "5"))

try:
    data = json.loads(os.environ.get("TOOL_INPUT_ENV") or "{{}}")
except (json.JSONDecodeError, TypeError):
    data = {{}}
...
current_iter = data.get("current_iteration", 1)
if isinstance(current_iter, str):
    try:
        current_iter = int(current_iter)
    except ValueError:
        current_iter = 1

match = re.search(r"<promise>(.*?)</promise>", last_msg)
if match and match.group(1) == expected:
    sys.exit(0)

if current_iter >= max_iter:
    sys.stderr.write(
        f"[{{agent}}] completion-check: marker absent but iteration "
        f"{{current_iter}} >= max {{max_iter}}; allowing termination.\\n"
    )
    sys.exit(0)

sys.stderr.write(
    f"[{{agent}}] completion-check: <promise>{{expected}}</promise> marker "
    f"not found in last_assistant_message (iteration {{current_iter}}/{{max_iter}}).\\n"
)
sys.stderr.write(
    "Re-prompting: emit the completion-promise marker when the goal is met.\\n"
)
sys.exit(2)
PYEOF
PY_EXIT=$?
...
def generate_stop_hook_completion_check_config(
    agent_name: str,
    script_path: str,
) -> dict[str, Any]:
    """Generate Claude Code settings.json hook config for the completion-check Stop hook.

    Args:
        agent_name: Agent name — validated for non-empty at the API boundary.
        script_path: Absolute path to the completion-check script.
